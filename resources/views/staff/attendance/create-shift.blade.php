...
<div class="flex items-center justify-between">
    <h2 class="font-semibold text-2xl sm:text-3xl text-gray-800 leading-tight">Create Shift</h2> 
    ...
    <form action="{{ route('staff.attendance.shifts.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}"
                   class="mt-2 w-full border-gray-300 rounded-lg shadow-sm text-sm sm:text-base focus:ring-green-500 focus:border-green-500"
                   required>
        </div>
        ...
        <div class="flex justify-end">
            <button type="submit"
                    class="px-6 py-3 bg-green-600 text-white text-sm sm:text-base rounded hover:bg-green-700">
                Save Shift
            </button>
        </div>
    </form>
