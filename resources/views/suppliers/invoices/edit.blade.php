...
<div class="w-full mx-auto bg-white shadow rounded-lg p-4 sm:p-6 space-y-6">
    ...
    <form method="POST" action="{{ route('suppliers.invoices.update', [$supplierId, $invoice]) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="invoice_date"
                       value="{{ old('invoice_date', $invoice->invoice_date ? \Carbon\Carbon::parse($invoice->invoice_date)->format('Y-m-d') : '') }}" required
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="3" placeholder="Provide updated ledger details, transaction notes, or balance modifications..."
                          class="w-full rounded-md border-gray-300 text-sm sm:text-base px-3 py-2 focus:ring-green-500 focus:border-green-500 shadow-sm">{{ old('notes', $invoice->notes) }}</textarea>
            </div>
        </div>
