...
<body class="min-h-screen antialiased bg-gray-900 select-none">
    ...
    <div class="relative min-h-screen w-full flex flex-col justify-between items-center overflow-y-auto">
        ...
        <div class="relative z-10 w-full flex flex-col items-center justify-center flex-grow text-center text-white px-6 py-12 max-w-4xl mx-auto space-y-6 sm:space-y-8">
            ...
            <div class="transform hover:scale-105 transition-transform duration-300 ease-out active:scale-95">
                <img src="{{ asset('storage/' . (\App\Models\Setting::where('setting_key','clinic_logo')->value('value') ?? 'logo.png')) }}" 
                     ...
                     class="w-24 h-24 sm:w-32 sm:h-32 object-contain drop-shadow-xl mx-auto">
            </div>

            <div class="space-y-3 sm:space-y-4">
                <h1 class="text-3xl sm:text-5xl font-bold tracking-tight leading-tight px-2">
                    Welcome to <span class="text-green-400">{{ \App\Models\Setting::where('setting_key','clinic_name')->value('value') ?? 'Supreme Clinic' }}</span>
                </h1>
                <p class="text-base sm:text-xl text-gray-300 max-w-2xl mx-auto font-light leading-relaxed px-4">
                    {{ \App\Models\Setting::where('setting_key','clinic_tagline')->value('value') ?? 'Your trusted partner in compassionate healthcare.' }}
                </p>
            </div>

            <div class="pt-2 w-full max-w-xs sm:max-w-none flex justify-center">
                <a href="{{ route('login') }}" 
                   class="inline-flex items-center justify-center px-8 py-3.5 bg-green-600 text-white rounded-lg text-lg sm:text-xl font-medium tracking-wide shadow-lg shadow-green-950/50 hover:bg-green-500 active:scale-98 transform transition-all duration-150 w-full sm:w-auto">
                    <i class="fas fa-sign-in-alt mr-2.5 text-base sm:text-lg opacity-90"></i> Login to System
                </a>
            </div>

            <div class="w-full max-w-md pt-4 sm:pt-6 space-y-2.5 text-sm sm:text-base text-gray-200 border-t border-white border-opacity-10 px-4">
                ...
                <p class="flex items-center justify-center gap-2.5">
                    <i class="fas fa-phone text-green-400 w-5 text-center"></i> 
                ...
                <p class="flex items-center justify-center gap-2.5 break-all">
                    <i class="fas fa-envelope text-blue-400 w-5 text-center"></i> 
                    <span>{{ \App\Models\Setting::where('setting_key','clinic_email')->value('value') ?? 'info@supremeclinic.ug' }}</span>
                </p>
                <p class="flex items-center justify-center gap-2.5">
                    <i class="fas fa-clock text-yellow-400 w-5 text-center"></i> 
                    <span>{{ \App\Models\Setting::where('setting_key','clinic_hours')->value('value') ?? 'Open 24/7' }}</span>
                </p>
            </div>
        </div>

        <footer class="relative z-10 w-full max-w-4xl mx-auto px-6 pb-6 pt-4 border-t border-gray-800 border-opacity-60 text-center space-y-3 mt-auto">
            
            <div class="flex items-center justify-center gap-6 text-xl sm:text-2xl">
            ...
            <p class="text-[11px] sm:text-xs text-gray-400 leading-relaxed max-w-xl mx-auto font-light">
                &copy; {{ date('Y') }} 
